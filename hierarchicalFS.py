...
class Memory(LoggingMixIn, Operations):
    ...
    def read(self, path, size, offset, fh):
        ...
        return parentDict['content'][targetFile]['data'][offset:offset + size]
    ...
    def readlink(self, path):
        parentDict, targetFile = GetParentDir_TargetFile(self, path)
        return parentDict['content'][targetFile]['data']
    ...
    def symlink(self, target, source):
        ...
        target_parentDict['content'][target_targetFile] = dict(st_mode=(S_IFLNK | 0o777), st_nlink=1,
                                                               st_size=len(source), content = '')
        target_parentDict['content'][target_targetFile]['data'] = source
    ...
    def truncate(self, path, length, fh=None):
        parentDict, targetFile = GetParentDir_TargetFile(self, path)
        parentDict['content'][targetFile]['data'] = parentDict['content'][targetFile]['data'][:data]
    ...
    def write(self, path, data, offset, fh):
        parentDict, targetFile = GetParentDir_TargetFile(self, path)
        parentDict['content'][targetFile]['data'] = parentDict['content'][targetFile]['data'][:offset] + data
        parentDict['content'][targetFile]['st_size'] = len(parentDict['content'][targetFile]['data'])
        return len(data)
